...
class findCluedo():
        ...
        if len(contours[0]) > 0:
            ...
            if cv2.contourArea(max_contour) > 350 and cv2.contourArea(max_contour) < 35000:
                ...
                if shape == "target":
                    ...
                    cv2.putText(cv_image, shape, (center_x, center_y), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (100,255,100), 2)
        ...
        while self.start and (time.time() - start_t) <= 60:
            ...
            fsm = {
                0: {
                    ...
                    "second condition": self.card_detected,
                    "next_state": [0,1,4]
                    },
                1: {
                    "output": self.approach_output,
                    "condition": self.point_reached,
                    "second condition": not self.frame_detected,
                    "next_state": [1,2,0]
                    },
                2: {
                    "output": self.wait_output,
                    "condition": self.card_detected,
                    "second condition": not self.point_reached,
                    "next_state": [3,4,1]
                    },
                3: {
            ...
            # Take output and conditional data from current state
            ...
            if second_condition == True:
                ...
                self.current_state = state.get("next_state")[1]
            else:
                self.current_state = state.get("next_state")[0]
                
            # Run current state output 
            output()
    ...
    def wait_output(self):
        ...
        counter = 0
        for i in range(0, 20):
            # detected = random.random() < 0.1 # for test purposes
            req = TriggerRequest()
            res = self.cluedo_identifier_client(req)
            # if detected == True: # for test purposes
            if res.success:
                counter = counter + 1
            rate.sleep()
